...
const sendPhotoRequest = catchAsync(async (req: Request, res: Response) => {
...
const handlePhotoRequest = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user.id;
  const { id } = req.params;
  const { status } = req.body;

  const result = await PhotoRequestService.handlePhotoRequest(userId, id as string, status);
...
const getReceivedPhotoRequests = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user.id;
...
const getSentPhotoRequests = catchAsync(async (req: Request, res: Response) => {
  const userId = (req as any).user.id;
